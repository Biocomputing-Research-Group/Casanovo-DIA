...
from ..utils import dict_to_pkl

logger = logging.getLogger("casanovo_DIA")
    ...
    **kwargs : Dict
    ...
    def _forward_step(
        ...
        tokens : torch.Tensor of shape (n_spectra, length)
            The predicted tokens for each spectrum.
        """
        return self.decoder(sequences, precursors, *self.encoder(spectra, ms1, ms2_b_ion, ms2_y_ion))
            ...
            if len(peptide_pred) > 0:
                ...
                if "$" not in peptide_pred and len(peptide_pred) > 0:
                    ...
                    peptide,aa_tokens,peptide_score,aa_scores = self._get_output_peptide_and_scores(
                        peptide_pred, aa_score
                    )
                    peptide_pred = ''.join(peptide_pred)
                    if self.out_writer:
    ...
    def predict_step(
        ...
        batch : Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]
            A batch of (i) MS/MS spectra, (ii) ms1 (elution profile), (iii) ms2_b_ion product ion peptide (v) ms2_y_ion (iv)precursor information, (            (vi) spectrum identifiers as torch Tensors.
